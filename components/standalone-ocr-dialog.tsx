...
export function StandaloneOCRDialog({ isOpen, onClose }: StandaloneOCRDialogProps) {
  const [pages, setPages] = useState<ProcessedPage[]>([])
  const [isProcessing, setIsProcessing] = useState(false)
  const { toast } = useToast()
  const [selectedLanguage, setSelectedLanguage] = useState<Language>(SUPPORTED_LANGUAGES[0]) // Default to Arabic
        ...
        <div className="space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground">Support for JPG, PNG, and other image formats</p>
                  </label>
                </div>
              </CardContent>
            </Card>
          ...
          {pages.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">Upload some scanned images to get started with OCR processing</p>
            </div>
          )}
        </div>
      </DialogContent>
